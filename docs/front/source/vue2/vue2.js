...
(function (global, factory) {
  ...
  ***************************************************************************** */
  ...
  function renderMixin(Vue) {
    ...
    // 返回值：新Vnode, 在renderWatcher调用 _update函数时，新Vnode和prevVnode经过patch函数进行diff算法，精确更新Dom
    Vue.prototype._render = function () {
      ...
      try {
        ...
        // vm.$createElement 是h函数,在这将h函数注入到new Vue(h)的，本质上就是vm.$createElement
  ...
  function lifecycleMixin(Vue) {
    ...
    // 通过patch进行diff算法对比，并生成和更新实际dom,真正的视图更新是在patch中完成的
  ...
  function initMixin$1(Vue) {
    Vue.prototype._init = function (options) {
      ...
      // 有proxy的时候会对render做一层代理，去做一些异常提醒，如果没有Proxy则 直接将vm的render作为 _renderProxy
      initProxy(vm);
      // expose real self
      vm._self = vm;
      // 初始化生命周期状态，并且确定该vm的 $parent，$root,以及将它添加到对应的$parent中
      initLifecycle(vm);
      // 初始化_event,如果有listener的话，去执行updateComponentListener
      initEvents(vm);
      // 将_c,createElement函数挂到vm上，解析插槽和作用域插槽，并且对vm的$attr和$listeners做响应式处理
      initRender(vm);
